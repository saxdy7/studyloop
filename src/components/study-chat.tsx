                ...
                <button
                  onClick={() => {
                    ...
                    const messagesHtml = messages
                      .map(
                        ...
                        <div style="margin-bottom: 20px; padding: 15px; border-radius: 8px; background: ${
                          m.role === "user" ? "#f5f5f5" : "#ffffff"
                        }; border: 1px solid #e0e0e0;">
                          <strong style="color: #111111; font-size: 12px; text-transform: uppercase;">
                            ${m.role === "user" ? "Student" : "LoopBot AI"}
                          </strong>
                          <div style="margin-top: 5px; font-size: 14px; line-height: 1.6; color: #333333; white-space: pre-wrap;">
                            ${m.content.replace(/<think>[\s\S]*?<\/think>/i, "")}
                          </div>
                        </div>
                      `
                      )
                    ...
                    printWindow.document.write(`
                      <html>
                        <head>
                          <title>StudyLoop Notes & Chat - ${activeSession.title}</title>
                          <style>
                            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; padding: 40px; background: #ffffff; color: #111111; }
                            h1 { font-size: 24px; border-bottom: 2px solid #111111; padding-bottom: 10px; margin-bottom: 30px; }
                            .metadata { margin-bottom: 30px; font-size: 13px; color: #666666; }
                            .footer { margin-top: 50px; font-size: 11px; text-align: center; color: #999999; border-top: 1px solid #eeeeee; padding-top: 20px; }
                          </style>
                        </head>
                        <body>
                          <h1>Study Session: ${activeSession.title}</h1>
                          <div class="metadata">Exported from StudyLoop on ${new Date().toLocaleDateString()}</div>
                          <div>${messagesHtml}</div>
                          <div class="footer">Generated by StudyLoop AI - Master Your Concepts</div>
                          <script>
                  ...
                  className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 px-3 py-1.5 text-[11px] font-semibold text-white transition-all cursor-pointer"
                >
